...
def minmax(df):
    for column in df.columns:
            if (((df[column].dtype == np.int64) or (df[column].dtype == np.float64)) or (column=='Valence' or column=='Arousal' or column=='Dominance') and not(column=="Video")):#if not(df[column].dtype == np.object) and not(column=="Video"): # : Fails for integers
...
def SelectTargetEmotions(df,TargetEmotionsList=pd.DataFrame()):
    ...
    # data = df.loc[(df['Target_Emotion'] == 'happiness') | (df['Target_Emotion'] == 'sadness') | (df['Target_Emotion'] == 'calmness') |(df['Target_Emotion'] == 'excitement')].copy()
    data = df.loc[(df['Target_Emotion'] == 'calmness') |(df['Target_Emotion'] == 'excitement')].copy()
    data['Stress_bin'] = data['Target_Emotion'].map({
                                                     "calmness":0,
                                                     "excitement":1})
    # data['Stress_bin'] = data['Target_Emotion'].map({"calmness":0, 
    #                                                  "surprise":1, 
    #                                                  "amusement":2, 
    #                                                  "fear":3, 
    #                                                  "excitement":4, 
...
def BuildKFold(data,n=10,features=pd.DataFrame(),Biosignal_Option='both'):
    ...
    selected_channels =['F7','F8','T7','T8']#['F7','F8','T7','T8']
    group_kfold = GroupKFold(n_splits=n)
    selected_features = []
    if selected_channels:
        for channel in selected_channels:
            selected_features+=['psdtheta_'+str(CHANNEL_IND[channel]),'psdalpha_'+str(CHANNEL_IND[channel]),'psdbeta_'+str(CHANNEL_IND[channel])]
    else:
        selected_features = ['psdtheta_1','psdalpha_1','psdbeta_1','psdtheta_2','psdalpha_2','psdbeta_2','psdtheta_3','psdalpha_3','psdbeta_3','psdtheta_4','psdalpha_4','psdbeta_4','psdtheta_5','psdalpha_5','psdbeta_5','psdtheta_6','psdalpha_6','psdbeta_6','psdtheta_7','psdalpha_7','psdbeta_7','psdtheta_8','psdalpha_8','psdbeta_8','psdtheta_9','psdalpha_9','psdbeta_9','psdtheta_10','psdalpha_10','psdbeta_10','psdtheta_11','psdalpha_11','psdbeta_11','psdtheta_12','psdalpha_12','psdbeta_12','psdtheta_13','psdalpha_13','psdbeta_13','psdtheta_14','psdalpha_14','psdbeta_14']
    if Biosignal_Option=='eeg':
        X = data.loc[:,selected_features].to_numpy()
